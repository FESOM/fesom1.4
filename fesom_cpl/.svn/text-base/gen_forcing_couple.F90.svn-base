     ...
     do i=1,nsend
         ...
         if (i.eq.1) then
            do n=1,myDim_nod2D+eDim_nod2D
               exchange(n)=tracer(nod3D_below_nod2D(1, n),1)       ! sea surface temperature deg C
            end do
            elseif (i.eq.2) then
...
subroutine read_chlorophyll
  ...
  if (firstcall .and. mype==0) then
     allocate(aux(nod2D))  
     file=trim(MeshPath)//'Chl_Sweeney.out'     
     open(51,file=trim(file))
     !file=trim(MeshPath)//'Chl_Sweeney.dat'     
     file='Chl_Sweeney.dat'     
     open(52,file=trim(file),form='unformatted', access='direct', recl=readtype*nod2d)
     
     write(*,*) 'Chlorophyll conversion:'
     do itime=1, 12
        do n2=1, nod2D
	   read(51, *) aux(n2)
	end do
        write(*,*) 'month=', itime, '; min=', minval(aux), '; max=', maxval(aux)

	write (52,rec=itime) aux
     end do
     close(51)
     close(52)
     write(*,*) 'validation:'
...
end subroutine read_chlorophyll
!
!---------------------------------------------------------------------------------------------------
!
...
SUBROUTINE force_flux_consv(field2d, mask, n, h, do_stats)
  ...
  use g_parfe, 		 only : myDim_nod2D, eDim_nod2D, mype
  use o_mesh,		 only :	geolat2d  
  use cpl_driver,	 only : nrecv, cpl_recv
  use cpl_exchange_mesh, only : a2o_fcorr_stat   
  use g_config,          only : istep
  IMPLICIT NONE
  ...
  rmask=mask
  SELECT CASE(h)
	   CASE( 1 ) ; where (geolat2d< 0) !just NH 
	                     rmask=0. 
		       end where
	   CASE( 2 ) ; where (geolat2d>=0) !just SH 
	                     rmask=0. 
		       end where
	   CASE DEFAULT 
	   !keep global mask
...
SUBROUTINE compute_residual(field2d, mask, n)
  ...
  use g_forcing_arrays,	only : 	atm_net_fluxes_north, atm_net_fluxes_south, 	&
				...
				flux_correction_north, flux_correction_south,	&
				flux_correction_total
  use g_parfe, 		only : 	myDim_nod2D, eDim_nod2D
...
SUBROUTINE integrate_2D(flux_global, flux_local, eff_vol, field2d, mask)
  ...
  use g_parfe !myDim_nod2D, eDim_nod2D, MPI stuff
  use o_elements,	only :	lump2d_north, lump2d_south
